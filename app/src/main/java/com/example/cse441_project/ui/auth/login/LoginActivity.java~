...
public class LoginActivity extends AppCompatActivity {
    ...
    private void observeViewModel() {
        viewModel.loginStatus.observe(this, isSuccess -> {
            if (isSuccess) {
                // Giả sử viewModel đã trả về userId sau khi đăng nhập thành công
                String userId = viewModel.getUserId(); // Phương thức này sẽ trả về ID người dùng
                Log.e("CheckoutActivity", "bookTicket: " +userId );
                // Lưu ID người dùng vào SharedPreferences
                SharedPreferences sharedPreferences = getSharedPreferences("MyAppPrefs", MODE_PRIVATE);
                SharedPreferences.Editor editor = sharedPreferences.edit();
                editor.putString("USER_ID", userId); // Lưu ID người dùng
                editor.apply();
